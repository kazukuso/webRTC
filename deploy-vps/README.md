...
- 利用者: `https://103.96.112.21/user.html`（端末コード **1234**）
- 通訳者: `https://103.96.112.21/staff.html?role=interpreter`
- 案内人: `https://103.96.112.21/staff.html?role=guide`

別の端末・回線からアクセスして、3者通話・言語ルーティング・録画を確認する。
録画は `deploy-vps/recordings/` にMP4が出力される。

> wss は同一オリジン(443)。最初にページの自己署名証明書を受け入れれば通話(wss)も通る。

## 7.5 管理コンソール（/admin.html）

- URL: `https://103.96.112.21/admin.html`
- ログイン: `.env` の **ADMIN_PASSWORD**（初期 `admin1234`。必ず変更）
- タブ: リアルタイム監視 / 通話履歴（CSV出力）/ 稼働レポート / マスタ管理（言語・端末）/ ログ（監査）
...
- **地域（ざっくり地名）**: 接続元IPからオフラインDB(geoip-lite)で都市/地域レベルを推定し、履歴・監視・CSVに記録（外部API不使用、利用者の許可不要）。Caddy経由の `X-Forwarded-For` から実IPを取得するため公開環境で有効（ローカルや社内NATでは「ローカル/不明」）。VPN/モバイルは実際とズレることがある。**IPと概略地は個人情報に準じるため、同意・プライバシーポリシーで扱いを明記すること。**
